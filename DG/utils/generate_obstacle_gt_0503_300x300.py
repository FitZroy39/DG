...
def generate_gt_with_pcd(label_path, gt_path, save_path, pcd_path, pcd_save_path):
    ...
    for ts in timestamp:
        ...
        for box, obj_type in important_box:
            ...
            min_y = max(min_y, Y_MIN)
            max_y = min(max_y, Y_MAX - 1)
            grid_min_x = int((min_x - X_MIN) / GRID_X)#int((min_x - X_MIN) / GRID_X)
            grid_max_x = round((max_x - X_MIN) / GRID_X)#int((max_x - X_MIN) / GRID_X)
            grid_min_y = int((min_y - Y_MIN) / GRID_Y)#int((min_y - Y_MIN) / GRID_Y)
            grid_max_y = round((max_y - Y_MIN) / GRID_Y)#int((max_y - Y_MIN) / GRID_Y)
            #print(obj_type,box)
            #print((grid_min_x, grid_max_x))
            #print((min_x, max_x))
            #print((grid_min_y, grid_max_y))
        ...
        temp = np.array(temp)
        save_root = "/private/personal/linyuqi/gpu12/mmsegmentation/data/pc_bev_10classes0503_300x300/extra_labeled_bbox"
        ...
        """
            for i in range(grid_min_x, grid_max_x+1):
                for j in range(grid_min_y, grid_max_y+1):
                    ...
                    centerpointy = y[j][i] + GRID_Y/2
                    #print('**',centerpointx, centerpointy)
                    grid_intersect_box = False
                    if grid_pcd_cnt[i][j] > 0 and lt[0] != lb[0] and rt[0] != rb[0] and lt[0] != rt[0] and lb[0] != rb[0] and \
                            lt[1] != lb[1] and rt[1] != rb[1] and lt[1] != rt[1] and lb[1] != rb[1]:
                        ltx = x[j][i]
                        lty = y[j][i] + GRID_Y
                        rbx = x[j][i] + GRID_X
                        rby = y[j][i]

                        grid_intersect_box = is_line_intersect_gird(lt[0], lt[1], lb[0], lb[1], ltx, lty, rbx, rby) or \
                            is_line_intersect_gird(lb[0], lb[1], rb[0], rb[1], ltx, lty, rbx, rby) or \
                            is_line_intersect_gird(rt[0], rt[1], rb[0], rb[1], ltx, lty, rbx, rby) or \
                            is_line_intersect_gird(lt[0], lt[1], rt[0], rt[1], ltx, lty, rbx, rby)
                    if check_in_polygon(box, centerpointx, centerpointy) or grid_intersect_box:
        ...
        gt_data = np.loadtxt(gt_name, dtype=np.int32)  # [:,:2]
        gt_grids = [tuple(gg[:2]) for gg in gt_data if gg[2] == 1 and gg[4] == 0]
...
factor_x = 1 / VOXEL_X_SIZE * BEV_FACTOR
factor_y = 1 / VOXEL_Y_SIZE * BEV_FACTOR
data_path = '/private/personal/linyuqi/grid_benchmark0503_300x300'
...
bag_name_list = []#bag_name_list0[275:]#[:1]#[]
for bag in bag_name_list0:
    if bag.startswith('howo4') or bag.startswith('howo7') or bag.startswith('howo8') or bag.startswith('howo13') or bag.startswith('howo16'):
        #continue
        bag_name_list.append(bag)
...
for idx,bag_name in enumerate(bag_name_list):#(bag_name_to_path_dict.keys()):#(bag_name_list):
    ...
    obstacle_gt_save_path = os.path.join(data_path, bag_name, 'obstacle_gt_fine')
    ...
    if not os.path.exists(obstacle_gt_save_path):
        os.makedirs(obstacle_gt_save_path)
    '''
    ...
    if idx < 1000:
        ...
        vis_save_path = os.path.join(data_path, bag_name, 'vis_gt_fine')
        if not os.path.exists(vis_save_path):
            os.makedirs(vis_save_path)
        show_list = file_list[::20]
        #ori_pcd_save_path = os.path.join(data_path, bag_name, 'pcd')
        for file in tqdm(show_list):
            pcd_path = os.path.join(pcd_save_path, file+'.bin')#(ori_pcd_save_path, file+'.pcd')#os.path.join(pcd_root, file+'.pcd')
