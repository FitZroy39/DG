...
@numba.jit(cache=True, fastmath=True)#@numba.jit(nopython=True)
def _points_to_voxel_reverse_kernel(points,
    ...
    failed = False
    for i in range(N):
        if self_car_extend is not None and self_car_extend[0][0] < points[i, 0] < self_car_extend[0][1] and \
            self_car_extend[1][0] < points[i, 1] < self_car_extend[1][1]:
            continue
        failed = False
        for j in range(ndim):
...
def points_to_voxel(points,
    ...
    if reverse_index:
        voxel_num = _points_to_voxel_reverse_kernel(
            ...
            coor_to_voxelidx, voxels, coors, max_points, max_voxels, np.array(self_car_extend))
